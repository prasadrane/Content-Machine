...
def _build_oracle_components(*, cfg, router, db_conn, connectors):
    """Assemble IdeaScorer + OracleOrchestrator via the shared bootstrap.

    Passes this module's top-level IdeaScorer / OracleOrchestrator names at
    call time — tests monkeypatch those exact names.
    """
    from content_machine.bootstrap import build_oracle_components

    return build_oracle_components(
        cfg=cfg,
        router=router,
        db_conn=db_conn,
...
def _cmd_oracle(args: argparse.Namespace) -> int:
    ...
    orch, _scorer = _build_oracle_components(
        cfg=cfg, router=router, db_conn=db_conn, connectors=connectors,
    )
